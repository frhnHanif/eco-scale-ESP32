...
void setup() {
  Serial.begin(115200);
  Serial.println("\nStarting Firmware (No NTP Version)...");
...
bool sendToLaravel() {
  ...
  if (httpResponseCode > 0) {
     ...
     if (httpResponseCode == 200 || httpResponseCode == 201 || response.indexOf("berhasil") >= 0) {
        Serial.println("✅ Database OK (Saved with Server Time)");
        success = true;
     } else {
